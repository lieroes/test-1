<script lang="ts">
	...
	const { form, errors, submitting, enhance } = superForm(data.sellForm);

	$: $exchangeStore.exchangeRate = data.exchangeRate;
	$: textContent = data.langTexts.pages.sell;

	onMount(() => {
		$exchangeStore.resetAmount();
		if (data.amount) $exchangeStore.to.update(data.amount);
	});
</script>
...
<div class="hero-section">
	<div class="hero-text">
		...
		<div class="exchange-flow-select">
			<a href="/" class="exchange-flow">{textContent.actionButtons.buy}</a>
			<img src="/images/change.svg" alt="Buy Icon" class="flow-icon" />
			<a href="/sell" class="exchange-flow exchange-flow--active"
				>{textContent.actionButtons.sell}</a
			>
		</div>
	</div>
	...
	<div class="exchange-form-container">
		<form action="/sell" method="post" use:enhance>
			...
			<input
				type="hidden"
				name="serviceFee"
				value={parseFloat($exchangeStore.to.amount) * serviceFee}
			/>
			<input
				type="hidden"
				name="networkFee"
				value={parseFloat($exchangeStore.from.amount) * networkFee}
			/>
			...
			<div class="form-layout">
				...
				<CurrencySelect
					...
					error={parseFloat($exchangeStore.to.amount) >= maxExchangeLimit[data.toCurrency]
						? `Amount must be less than ${maxExchangeLimit[data.toCurrency].toFixed(2)} ${
								data.toCurrency
						  }`
						: null}
					isFiat={true}
				...
				<div class="form-layout__action">
					<button
						...
						disabled={!(
							parseFloat($exchangeStore.to.amount) > 0 &&
							parseFloat($exchangeStore.to.amount) <= maxExchangeLimit[data.toCurrency] &&
							parseFloat($exchangeStore.to.amount) >= minExchangeLimit[data.toCurrency]
						)}
					>
						{textContent.button}
					</button>
			...
			<div class="commission-details__list">
				<section class="commission">
					...
					<div class="light-purple-text">
						{(Number($exchangeStore.to.amount) * 0.04).toFixed(2)}
				...
				<section class="commission">
					...
					<div class="light-purple-text">
						{(Number($exchangeStore.from.amount) * 0.02).toFixed(7)}
...
<LargeCta />
<Guide />
<ExchangeRatesCarousel currencies={data.cryptoCurrencies} currencyCode={data.toCurrency} />
<Packages currencies={data.cryptoCurrencies} currencyCode={data.toCurrency} />
<SmallCta />
<Features />
<FrequentQuestions />
